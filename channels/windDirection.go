/*
	In mmessage passing we dont have to worry about shared bits of memory as in message passing you producer
	which is sending the message to the consumer.
	...
	Default behaviour of channel is that operation is blocking until it has been read by the consumer

	Producer										Consumer

	channel <-- "hello"								running
	blocked 										running
	running 										x <-- channel

	In this example we will be running a pipeline kind of structure

	Read the File ---> Parse ---> Feature Extraction ---> Data Mining
	T1					T2			T3						T4				<-- Threads
...
func main() {
	...
	absPath, _ := filepath.Abs("C:/Users/Acer/go/src/github.com/Concurrency-Go/metarfiles/")
